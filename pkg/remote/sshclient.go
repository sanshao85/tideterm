...
import (
	...
	"github.com/sanshao85/tideterm/pkg/trimquotes"
	"github.com/sanshao85/tideterm/pkg/userinput"
...
func ConnectToClient(connCtx context.Context, opts *SSHOpts, currentClient *ssh.Client, jumpNum int32, connFlags *wconfig.ConnKeywords) (*ssh.Client, int32, error) {
	...
	if jumpNum > SshProxyJumpMaxDepth {
		return nil, jumpNum, ConnectionError{ConnectionDebugInfo: debugInfo, Err: fmt.Errorf("ProxyJump %d exceeds TideTerm's max depth of %d", jumpNum, SshProxyJumpMaxDepth)}
	}
