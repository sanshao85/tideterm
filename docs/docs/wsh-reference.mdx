...
wsh setmeta -b [blockid] url=https://github.com/sanshao85/tideterm
...
The `web` command opens URLs in a web block within TideTerm.
...
```sh
# Open a URL
wsh web open https://github.com/sanshao85/tideterm
...
The `notify` command creates a desktop notification from TideTerm.
...
TideTerm provides commands for managing persistent variables at different scopes (block, tab, workspace, or client-wide).
...
The `appath` command lets you get the paths to various TideTerm directories and files, including configuration, data storage, and logs.

```sh
wsh appath {config|data|log}
```

Aliases: `wavepath`, `tidpath`

This command returns the full path to the requested TideTerm system directory or file. It's useful for accessing TideTerm's configuration files, data storage, or checking logs.
...
- `config` - Where TideTerm stores its configuration files
- `data` - Where TideTerm stores its persistent data
- `log` - The main TideTerm log file
