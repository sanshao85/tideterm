...
TideTerm's configuration files live in TideTerm's config directory. You can locate it with:

```sh
wsh appath config
```

On Linux this is typically `~/.config/tideterm/`.

The main configuration file is `settings.json` in that directory.
...
The easiest way to edit your config files is to use the `wsh editconfig` command, which will open your TideTerm config file in the built-in preview editor.
...
| ------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| app:globalhotkey                     | string   | A systemwide keybinding to open your most recent TideTerm window. This is a set of key names separated by `:`. For more info, see [Customizable Systemwide Global Hotkey](#customizable-systemwide-global-hotkey)                                              |
| app:dismissarchitecturewarning       | bool     | Disable warnings on app start when you are using a non-native architecture for TideTerm. For more info, see [Why does Wave warn me about ARM64 translation when it launches?](./faq#why-does-wave-warn-me-about-arm64-translation-when-it-launches).          |
...
```json
{
    ...
    "web:defaulturl": "https://github.com/sanshao85/tideterm",
...
If you previously used the upstream Wave Terminal, its configuration files may live under `~/.waveterm/`. TideTerm uses separate config/data locations to avoid conflicting with an existing Wave installation.

:::

## Environment Variable Resolution

To avoid putting secrets directly in config files, TideTerm supports environment variable resolution using `$ENV:VARIABLE_NAME` or `$ENV:VARIABLE_NAME:fallback` syntax. This works for any string value in any config file (settings.json, presets.json, ai.json, etc.).
...
```json
{
  ...
  "bookmark@tideterm": {
    "url": "https://github.com/sanshao85/tideterm",
    "title": "TideTerm",
    "display:order": -1
  },
  "bookmark@tideterm-github": {
    "url": "https://github.com/sanshao85/tideterm",
    "title": "TideTerm GitHub",
  ...
  "bookmark@tideterm-pulls": {
    "url": "https://github.com/sanshao85/tideterm/pulls",
    "title": "TideTerm Pull Requests",
    "iconurl": "https://github.githubassets.com/favicons/favicon-dark.png"
  }
}
```
...
User-defined terminal themes are located in `termthemes.json` in TideTerm's config directory (see `wsh appath config`).
...
Here is an example of defining a full terminal theme. All of the built-in themes are defined here: https://github.com/sanshao85/tideterm/blob/main/pkg/wconfig/defaultconfig/termthemes.json (if you'd like to add a popular terminal theme, please submit a PR!)
