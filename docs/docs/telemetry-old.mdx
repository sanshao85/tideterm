...
As soon as telemetry is disabled, TideTerm may send a special update that notifies the configured endpoint of this change. See [When Telemetry is Turned Off](#when-telemetry-is-turned-off) for more info. The timer still runs in the background but no data is sent.
...
| Name          | Description                                                                                                                                                                     |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ActiveMinutes | The number of minutes that the user has actively used TideTerm on a given day. This requires the terminal window to be in focus while the user is actively interacting with it. |
| FgMinutes     | The number of minutes that TideTerm has been in the foreground on a given day. This requires the terminal window to be in focus regardless of user interaction.                 |
| OpenMinutes   | The number of minutes that TideTerm has been open on a given day. This only requires that the terminal is open, even if the window is out of focus.                             |
...
| ClientVersion | Which version of TideTerm is installed.                                                                                                                                                                                                                                                          |
...
| ClientId          | This is an anonymous UUID created when TideTerm is first launched. It is used for telemetry and sending prompts to Open AI. |
...
When a user disables telemetry, TideTerm may send a notification that their anonymous _ClientId_ has had its telemetry disabled (if an endpoint is configured). This is done with the `wcloud.NoTelemetryInputType` type in the source code. Beyond that, no further information is sent unless telemetry is turned on again.
