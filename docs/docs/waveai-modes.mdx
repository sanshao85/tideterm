...
TideTerm supports custom AI modes that allow you to use local models, custom API endpoints, and alternative AI providers. This gives you complete control over which models and providers you use with TideTerm's AI features.

## Configuration Overview

AI modes are configured in `waveai.json` in TideTerm's config directory (see `wsh appath config`).

**To edit using the UI:**
1. Click the settings (gear) icon in the widget bar
2. Select "Settings" from the menu
3. Choose "AI Modes" from the settings sidebar
