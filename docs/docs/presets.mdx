...
TideTerm's preset system allows you to save and apply multiple configuration settings at once. Presets are used for:
...
- `<config-dir>/presets.json`: Main presets file (see `wsh appath config`)
- `<config-dir>/presets/`: Directory for organizing presets into separate files
...
The `preset-type` determines where the preset appears in TideTerm's interface:
