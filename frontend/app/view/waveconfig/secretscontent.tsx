...
const EmptyState = memo(({ onAddSecret }: { onAddSecret: () => void }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-4 py-12 h-full bg-zinc-800/50 rounded-lg">
            ...
            <h3 className="text-lg font-semibold text-zinc-400">{t("secrets.noSecrets")}</h3>
            <p className="text-zinc-500">{t("secrets.addToGetStarted")}</p>
            <button
            ...
            >
                ...
                <span className="font-medium">{t("secrets.addNewSecret")}</span>
            </button>
        </div>
    );
});
    ...
    return (
        <div className="flex flex-col gap-2 p-4 m-4 bg-zinc-800/50 rounded-lg">
            <div className="flex items-center gap-2">
                ...
                <div className="text-sm font-medium text-zinc-300">{t("secrets.cliAccess")}</div>
            </div>
        ...
        <div className="flex flex-col h-full w-full rounded-lg">
            <div className="flex flex-col divide-y divide-zinc-700">
                ...
                >
                    ...
                    <span className="font-medium text-accent-500">{t("secrets.addNewSecret")}</span>
                </div>
            </div>
            <CLIInfoBubble />
        </div>
    ...
    ({
        ...
        return (
            <div className="flex flex-col gap-4 min-h-full p-6 bg-zinc-800/50 rounded-lg">
                <h3 className="text-lg font-semibold">{t("secrets.addNewSecret.title")}</h3>
                <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium">{t("secrets.secretName")}</label>
                    <input
                        type="text"
                        className={cn(
                        ...
                        placeholder={t("secrets.secretName.placeholder")}
                        disabled={isLoading}
                    />
                    <div className="text-xs text-zinc-400">{t("secrets.secretName.help")}</div>
                </div>
                <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium">{t("secrets.secretValue")}</label>
                    <textarea
                        ...
                        placeholder={t("secrets.secretValue.placeholder")}
                        disabled={isLoading}
                        rows={4}
                    />
                </div>
        ...
        <div className="flex flex-col gap-4 min-h-full p-6 bg-zinc-800/50 rounded-lg">
            ...
            <div className="flex flex-col gap-2">
                ...
                <textarea
                    ...
                    disabled={isLoading}
                    rows={6}
                    placeholder={!secretShown ? t("secrets.secretValue.placeholderNew") : ""}
                />
                {!secretShown && (
                    <div className="text-sm text-zinc-400">
                        {t("secrets.currentValueNotShown")}{" "}
                        {isLoading ? (
            ...
            <div className="flex gap-2 justify-between">
                <button
                    ...
                    disabled={isLoading}
                    title={t("secrets.deleteThisSecret")}
                >
                    {isLoading ? (
                        <>
                            <i className="fa-sharp fa-solid fa-spinner fa-spin" />
...
export const SecretsContent = memo(({ model }: SecretsContentProps) => {
    ...
    if (isLoading && secretNames.length === 0 && !selectedSecret) {
        return (
            <div className="w-full h-full">
                <div>
                    <LoadingSpinner message={t("secrets.loadingSecrets")} />
                </div>
            </div>
        );
    }
